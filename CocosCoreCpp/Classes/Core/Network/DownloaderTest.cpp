...
void DownloaderTests::btnHandle(Ref* sender)
{
...
Node* DownloaderTests::createDownloadView(const char *name, const Widget::ccWidgetClickCallback &callback)
{
	...
	auto bg = Scale9Sprite::createWithSpriteFrameName("button_actived.png");
	bg->setContentSize(viewSize);
	// create background
	//auto bg = LayerColor::create(Color4B(125,0,0,125));
	//bg->setContentSize(viewSize);
	...
	auto btn = Button::create("animationbuttonnormal.png",
	...
	// add a progress bar
	auto bar = LoadingBar::create("silder_progressBar.png");
...
//void DownloaderTests::onExit()
//{
//	Layer::onExit();
//}
void DownloaderTests::onEnter()
{        
	Layer::onEnter();
	...
	Node* topRightView = createDownloadView(sNameList[0], [this](Ref*)
	{
		...
		auto btn = (Button*)view->getChildByTag(TAG_BUTTON);
	...
	topRightView->setPosition(getPositionCenter());
	...
	Node* topLeftView = createDownloadView(sNameList[1], [this](Ref*)
	{
		...
		btn->setEnabled(false);
		btn->setVisible(false);
		auto bar = (LoadingBar*)view->getChildByTag(TAG_PROGRESS_BAR);
		bar->setPercent(0);
		bar->setVisible(true);
		bar->setEnabled(true);
	...
	topLeftView->setPosition(getPositionCenter());
	...
	bottomLeftView->setPosition(getPositionCenter());
	...
	bottomRightView->setPosition(getPositionCenter());
...
Vec2 DownloaderTests::getPositionCenter()
{
	return Vec2(500,500);
}