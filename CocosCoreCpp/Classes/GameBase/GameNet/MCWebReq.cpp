...
#include "MCWebReq.h"
#include "GameBase/Tools/utility.h"
#include "GameBase/Tools/ScriptData.h"
...
MCWebReq::MCWebReq()
{
 ...
 void MCWebReq::sendRequestStr(std::string kUrl,std::function<void(std::string)> _callBack1)
 {
	 kUrl = utility::toString(getWebIp(),kUrl);
	 sendRequestDocumentUrl(kUrl,nullptr,_callBack1);
 }
 void MCWebReq::sendRequestDocument(std::string kUrl,std::function<void(rapidjson::Document*)> _callBack1)
 {
	 kUrl = utility::toString(getWebIp(),kUrl);
	 sendRequestDocumentUrl(kUrl,_callBack1,nullptr);
 }
 void MCWebReq::sendRequestDocumentUrl(std::string kUrl,std::function<void(rapidjson::Document*)> _callBack1,std::function<void(std::string)> _callBack2)
 {
 ...
 void MCWebReq::sendFile(std::string kUrl, std::string &fileName,const cocos2d::network::ccHttpRequestCallback &callback)
 {
 ...
 void MCWebReq::sendFileURLHeadImgaic(std::string kUrl, std::string &fileName,std::function<void(rapidjson::Document*)> _callBack)
 {
	 kUrl = utility::toString("http://",getWebIp(),"/"+kUrl);
	 cocos2d::network::HttpRequestEx *request = new cocos2d::network::HttpRequestEx();
	 request->setUrl(kUrl.c_str());
